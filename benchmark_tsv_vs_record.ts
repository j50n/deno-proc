...
function generateTestData(format: "tsv" | "record"): string {
  ...
  while (currentSize < targetSize) {
    const row =
      `field1_${rows.length}${fieldSep}field2_${rows.length}${fieldSep}field3_${rows.length}${fieldSep}field4_${rows.length}${fieldSep}field5_${rows.length}${recordSep}`;
    rows.push(row);
...
console.log(
  `Record data size: ${(recordData.length / (1024 * 1024)).toFixed(2)} MB`,
);
...
const tsvToLazyrow = await benchmark(
  "tsv2lazyrow",
  ["tsv2lazyrow"],
  "/tmp/test.tsv",
);
...
const recordToLazyrow = await benchmark(
  "record2lazyrow",
  ["record2lazyrow"],
  "/tmp/test.rec",
);
...
await run(
  ...
  "/tmp/test.rec",
  "-o",
  "/tmp/test_from_rec.lazy",
).status;
console.log();

console.log("=== LazyRow → TSV (WASM) ===");
const lazyrowToTsv = await benchmark(
  "lazyrow2tsv",
  ["lazyrow2tsv"],
  "/tmp/test_from_tsv.lazy",
);
...
const lazyrowToRecord = await benchmark(
  "lazyrow2record",
  ["lazyrow2record"],
  "/tmp/test_from_rec.lazy",
);
