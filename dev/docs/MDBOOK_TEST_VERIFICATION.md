...
This document verifies that all tests in `tests/mdbook_examples.test.ts`
accurately reflect the documentation.

## Verification Status: ✅ ALL VERIFIED

All 36 tests have been verified against their corresponding documentation
examples.
...
- **Code**:
  `read(tempFile).lines.filter(line => line.includes("ERROR")).count()`
- **Status**: EXACT MATCH
...
- **Documentation**: `site/src/iterables/enumerable.md` (implicit in
  array-methods)
...
- **Code**:
  `enumerate(["hello world", "foo bar"]).flatMap(line => line.split(" "))`
- **Status**: EXACT MATCH
...
- **Code**:
  `enumerate(items).reduce((acc, item) => { acc[item.category] = ...; })`
- **Status**: MATCHES PATTERN
...
- **Status**: EXACT MATCH (documentation was corrected to remove non-existent
  `.slice()` method)
...
- **Code**:
  `enumerate([1, 2, 3]).concurrentMap(async (n) => { ... }, { concurrency: 2 })`
- **Status**: EXACT MATCH
...
- **Code**:
  `read("war-and-peace.txt.gz").transform(new DecompressionStream("gzip")).lines.count()`
- **Status**: EXACT MATCH
...
- **Code**:
  `read("app.log").lines.filter(line => line.includes("ERROR")).count()`
- **Status**: EXACT MATCH
...
All tests accurately reflect the documentation. The documentation is now
verified to be correct and runnable.
