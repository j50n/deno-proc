...
Imagine you have a callback-based API (like event emitters, WebSocket messages,
or sensor data) and you want to process it with proc's pipeline operations. You
can't easily convert callbacks to an AsyncIterable... until now.
...
It uses an internal queue to buffer items between the writer and reader,
allowing them to operate at different speeds.
...
```typescript
import { sleep, WritableIterable } from "jsr:@j50n/proc@{{gitv}}";
...
This demonstrates the streaming nature: the reader receives items as they're
written, not all at once.

> **⚠️ Important**: You MUST call `.close()` when done writing, or iteration
...
with proc's pipeline operations.
