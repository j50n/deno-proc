...
function serializeStringRow(columns: string[]): Uint8Array {
  ...
  const positionsSize = positions.length << 2; // int32 positions
  ...
  // Write column count as int32
  view.setInt32(0, columns.length, true);
  
  // Write positions as int32
  for (let i = 0; i < positions.length; i++) {
    view.setInt32(4 + (i << 2), positions[i], true);
  }
