...
import {
  ...
  fromRecordToRows,
  toRecord,
} from "../../src/transforms/record.ts";
import {
  FIELD_SEPARATOR,
  RECORD_SEPARATOR,
} from "../../src/transforms/common.ts";

Deno.test("Record - basic parsing to RowData", async () => {
  const recordData =
    `name${FIELD_SEPARATOR}age${FIELD_SEPARATOR}city${RECORD_SEPARATOR}Alice${FIELD_SEPARATOR}30${FIELD_SEPARATOR}NYC${RECORD_SEPARATOR}Bob${FIELD_SEPARATOR}25${FIELD_SEPARATOR}LA${RECORD_SEPARATOR}`;
...
Deno.test("Record - empty fields", async () => {
  const recordData =
    `${FIELD_SEPARATOR}middle${FIELD_SEPARATOR}${RECORD_SEPARATOR}`;
...
Deno.test("Record - multi-byte UTF-8", async () => {
  const recordData =
    `北京${FIELD_SEPARATOR}中国${FIELD_SEPARATOR}🏙️${RECORD_SEPARATOR}`;
...
Deno.test("Record pathological - 1MB field", async () => {
  ...
  const recordData =
    `small${FIELD_SEPARATOR}${hugeField}${FIELD_SEPARATOR}end${RECORD_SEPARATOR}`;
...
Deno.test("Record pathological - many small chunks", async () => {
  const recordData =
    `a${FIELD_SEPARATOR}b${FIELD_SEPARATOR}c${RECORD_SEPARATOR}d${FIELD_SEPARATOR}e${FIELD_SEPARATOR}f${RECORD_SEPARATOR}`;
  const bytes = new TextEncoder().encode(recordData);
