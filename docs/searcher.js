...
(function search(search) {
  ...
  function formatSearchResult(result, searchterms) {
    ...
    // representation (%27).
    var searchterms = encodeURIComponent(searchterms.join(" ")).replace(
      /\'/g,
      "%27",
    );

    return '<a href="' + path_to_root + url[0] + "?" + URL_MARK_PARAM + "=" +
      searchterms + "#" + url[1] +
      '" aria-details="teaser_' + teaser_count + '">' + result.doc.breadcrumbs +
      "</a>" +
      '<span class="teaser" id="teaser_' + teaser_count +
      ...
      for (var wordindex in words) {
        ...
        if (word.length > 0) {
          for (var searchtermindex in stemmed_searchterms) {
            if (
              elasticlunr.stemmer(word).startsWith(
  ...
  function globalKeyHandler(e) {
    ...
    if (e.keyCode === ESCAPE_KEYCODE) {
      ...
      !hasFocus() && (e.keyCode === DOWN_KEYCODE ||
