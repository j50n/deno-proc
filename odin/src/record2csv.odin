...
// Convert record format to CSV with proper RFC 4180 quoting
// Returns: number of bytes written, or -1 if output buffer too small
@(export)
record_to_csv :: proc "c" (input_ptr: rawptr, input_len: int, output_ptr: rawptr, output_capacity: int, separator: u8, always_quote: i32, crlf: i32) -> int {
	...
	for i := 0; i <= input_len; i += 1 {
		is_field_sep := i < input_len && input[i] == FIELD_SEP
		is_record_sep := i < input_len && input[i] == RECORD_SEP
		is_end := i == input_len
		
		if is_field_sep || is_record_sep || (is_end && field_start < input_len) {
