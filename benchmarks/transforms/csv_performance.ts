...
function generateCsvData(rows: number): string {
  ...
  for (let i = 0; i < rows; i++) {
    data.push(
      `${i},User${i},user${i}@example.com,${25 + (i % 40)},City${
        i % 100
      },Country${i % 20},${30000 + (i % 70000)}`,
    );
  }
...
async function benchmarkCsvParsing(name: string, data: string, iterations = 3) {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const result = await enumerate([bytes]).transform(fromCsvToRows())
      .collect();
  ...
  for (let i = 0; i < iterations; i++) {
    const start = performance.now();
    const result = await enumerate([bytes]).transform(fromCsvToLazyRows())
      .collect();
    const end = performance.now();
  ...
  console.log(
    `fromCsvToRows:     ${avgRowTime.toFixed(2)}ms (${
      (dataSize / 1024 / 1024 / (avgRowTime / 1000)).toFixed(2)
    } MB/s)`,
  );
  console.log(
    `fromCsvToLazyRows: ${avgLazyTime.toFixed(2)}ms (${
      (dataSize / 1024 / 1024 / (avgLazyTime / 1000)).toFixed(2)
    } MB/s)`,
  );
  console.log(
    `LazyRow advantage: ${(avgRowTime / avgLazyTime).toFixed(2)}x faster`,
  );
}

async function benchmarkCsvStringify(
  name: string,
  rowCount: number,
  iterations = 3,
) {
  ...
  console.log(
    `toCsv: ${avgTime.toFixed(2)}ms (${
      (outputSize / 1024 / 1024 / (avgTime / 1000)).toFixed(2)
    } MB/s)`,
  );
