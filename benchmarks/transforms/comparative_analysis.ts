...
function dataToFormats(data: Record<string, string>[]) {
  ...
  const recordData =
    data.map((row) => Object.values(row).join(FIELD_SEPARATOR)).join(
      RECORD_SEPARATOR,
    ) + RECORD_SEPARATOR;
...
async function benchmarkRowParser(
  ...
  console.log(
    `${name.padEnd(20)} ${avgTime.toFixed(2).padStart(8)}ms ${
...
async function benchmarkRowStringifier(
  name: string,
  data: string[][],
  stringifier: () => (
...
async function _benchmarkLazyRowStringifier(
  testData: LazyRow[],
  stringifier: () => (
    input: AsyncIterable<LazyRow[]>,
  ) => AsyncIterable<Uint8Array>,
  format: string,
  iterations = 3,
) {
  ...
  for (let i = 0; i < iterations; i++) {
    const start = performance.now();
    const result = await enumerate([testData]).transform(stringifier())
      .collect();
    const end = performance.now();
    times.push(end - start);
    if (i === 0) {
...
async function benchmarkObjectStringifier(
  name: string,
  data: Record<string, string>[],
  stringifier: () => (
    input: AsyncIterable<Record<string, string>[]>,
  ) => AsyncIterable<Uint8Array>,
  iterations = 3,
) {
  const times: number[] = [];
...
async function runComparativeAnalysis() {
  ...
  for (const { name, rows } of testSizes) {
    ...
    console.log(
      `Data size: CSV ${(formats.csv.length / 1024).toFixed(1)}KB, TSV ${
        (formats.tsv.length / 1024).toFixed(1)
      }KB, JSON ${(formats.jsonl.length / 1024).toFixed(1)}KB, Record ${
        (formats.record.length / 1024).toFixed(1)
      }KB`,
    );
    ...
    const csvRows = await benchmarkRowParser(
      "CSV → Rows",
      formats.csv,
      fromCsvToRows,
    );
    const csvLazy = await benchmarkLazyRowParser(
      "CSV → LazyRows",
      formats.csv,
      fromCsvToLazyRows,
    );
    const tsvRows = await benchmarkObjectParser(
      "TSV → Rows",
      formats.tsv,
      fromTsvToRows,
    );
    const tsvLazy = await benchmarkLazyRowParser(
      "TSV → LazyRows",
      formats.tsv,
      fromTsvToLazyRows,
    );
    const jsonRows = await benchmarkRowParser(
      "JSON → Rows",
      formats.jsonl,
      fromJsonToRows,
    );
    const recordRows = await benchmarkRowParser(
      "Record → Rows",
      formats.record,
      fromRecordToRows,
    );
    const recordLazy = await benchmarkLazyRowParser(
      "Record → LazyRows",
      formats.record,
      fromRecordToLazyRows,
    );
    ...
    console.log(
      `CSV:    LazyRow ${
        (csvLazy.throughput / csvRows.throughput).toFixed(2)
      }x faster than Rows`,
    );
    console.log(
      `TSV:    LazyRow ${
        (tsvLazy.throughput / tsvRows.throughput).toFixed(2)
      }x faster than Rows`,
    );
    console.log(
      `Record: LazyRow ${
        (recordLazy.throughput / recordRows.throughput).toFixed(2)
      }x faster than Rows`,
    );
    ...
    parsers.forEach((parser, i) => {
      console.log(
        `${i + 1}. ${parser.name}: ${parser.throughput.toFixed(2)} MB/s`,
      );
    });
  }
}
