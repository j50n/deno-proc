...
function sanityCheck<T>(
  ...
  if (totalRows < expectedMinRows * 0.8) {
    console.warn(
      `⚠️  ${format}: Expected ~${expectedMinRows} rows, got ${totalRows}`,
    );
    return false;
  }
  return true;
...
async function benchmarkRowParser(
  ...
  console.log(
    ...
    } MB/s ${resultCount.toLocaleString().padStart(8)} rows`,
  );
...
async function benchmarkRowStringifier(
  ...
  console.log(
    `${avgTime.toFixed(2).padStart(8)}ms ${
      throughput.toFixed(2).padStart(8)
    } MB/s ${(outputSize / 1024 / 1024).toFixed(2).padStart(8)} MB`,
  );
...
async function runStreamingBenchmarks() {
  ...
  try {
    ...
    console.log(
      "ℹ️  Large datasets not found, skipping. Run ./benchmarks/data/generate_datasets.ts to create them.\n",
    );
  }
  ...
  for (const [filename, expectedRows, description] of testConfigs) {
    ...
    try {
      ...
      console.log(
        `File: ${filename} (${(stat.size / 1024 / 1024).toFixed(2)} MB)`,
      );
    } catch {
    ...
    if (csvRows) {
      ...
      const testRowData = Array.from(
        { length: sampleSize },
        (_, i) => Array.from({ length: 10 }, (_, j) => `field_${i}_${j}`),
      );
      const testObjectData = Array.from(
        { length: sampleSize },
        (_, i) =>
          Object.fromEntries(
            Array.from(
              { length: 10 },
              (_, j) => [`column_${j}`, `field_${i}_${j}`],
            ),
          ),
      );
      ...
      await benchmarkObjectStringifier(
        testObjectData,
        toJson,
        "Objects → JSON",
      );
