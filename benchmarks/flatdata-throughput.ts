...
async function generateTestData() {
  ...
  console.log(`Generating test data: ${NUM_RECORDS} records × ${NUM_COLUMNS} columns...`);
  
  // Generate CSV
  const csvLines: string[] = [];
  for (let i = 0; i < NUM_RECORDS; i++) {
    const fields = Array.from({ length: NUM_COLUMNS }, (_, j) => `field${j}_${i}`);
  ...
  const cmd = new Deno.Command("deno", {
    args: ["run", "--allow-read", "--allow-write", FLATDATA, "csv2lazyrow", "-i", `${TEMP_DIR}/test.csv`, "-o", `${TEMP_DIR}/test.lazy`],
    stdout: "piped",
    stderr: "piped",
  });
...
async function runBenchmarks() {
  ...
  const conversions = [
    // CSV conversions
    { name: "csv2record", command: "csv2record", input: `${TEMP_DIR}/test.csv` },
    { name: "csv2lazyrow", command: "csv2lazyrow", input: `${TEMP_DIR}/test.csv` },
    { name: "csv2tsv", command: "csv2tsv", input: `${TEMP_DIR}/test.csv` },
    
    // TSV conversions
    { name: "tsv2record", command: "tsv2record", input: `${TEMP_DIR}/test.tsv` },
    { name: "tsv2lazyrow", command: "tsv2lazyrow", input: `${TEMP_DIR}/test.tsv` },
    { name: "tsv2csv", command: "tsv2csv", input: `${TEMP_DIR}/test.tsv` },
    
    // Record conversions
    { name: "record2csv", command: "record2csv", input: `${TEMP_DIR}/test.rec` },
    { name: "record2tsv", command: "record2tsv", input: `${TEMP_DIR}/test.rec` },
    { name: "record2lazyrow", command: "record2lazyrow", input: `${TEMP_DIR}/test.rec` },
    
    // LazyRow conversions
    { name: "lazyrow2csv", command: "lazyrow2csv", input: `${TEMP_DIR}/test.lazy` },
    { name: "lazyrow2tsv", command: "lazyrow2tsv", input: `${TEMP_DIR}/test.lazy` },
    { name: "lazyrow2record", command: "lazyrow2record", input: `${TEMP_DIR}/test.lazy` },
  ];
  ...
  for (const { name, command, input } of conversions) {
    ...
    console.log(`${name.padEnd(20)} ${result.time.toFixed(3).padStart(7)}   ${result.throughput.toFixed(1).padStart(6)}`);
  }
  ...
  console.log(`  CSV conversions:     ${avgThroughput(csvConversions).toFixed(1)} MB/s avg`);
  console.log(`  TSV conversions:     ${avgThroughput(tsvConversions).toFixed(1)} MB/s avg`);
  console.log(`  Record conversions:  ${avgThroughput(recConversions).toFixed(1)} MB/s avg`);
  console.log(`  LazyRow conversions: ${avgThroughput(lazyConversions).toFixed(1)} MB/s avg`);
  console.log(`  Overall average:     ${avgThroughput(results).toFixed(1)} MB/s`);
}
