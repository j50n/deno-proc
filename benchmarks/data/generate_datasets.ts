...
const specialChars = ["café", "naïve", "🚀", "résumé", "Zürich", "東京", "москва"];
const cities = ["New York", "London", "Paris", "Tokyo", "Sydney", "Berlin", "São Paulo"];
...
function generateRow(rowIndex: number, columnCount: number): Record<string, string> {
  ...
  for (let col = 0; col < columnCount; col++) {
    ...
    if (col === 0) {
      ...
      value = Math.random() < 0.1 ? getRandomItem(specialChars) : getRandomItem(names);
      ...
      value = Math.random() < 0.05 ? getRandomItem(specialChars) : getRandomItem(cities);
    } else {
      // Regular field with occasional special chars
      value = Math.random() < 0.02 ? getRandomItem(specialChars) : `field_${rowIndex}_${col}`;
    }
...
async function writeDataset(filename: string, rows: number, columns: number) {
  ...
  console.log(`  Generated ${filename.replace('.csv', '.{csv,tsv,json}')}: ${csvSize} MB`);
}
